...
from .const import CONF_ABSENCE_DELAY, CONF_LUX_THRESHOLD, CONF_SLEEP_ABSENCE_DELAY, DOMAIN
from .entity import ArmEntity


async def async_setup_entry(hass, entry, async_add_entities):
    manager = hass.data[DOMAIN][entry.entry_id]
    async_add_entities([AbsenceDelay(manager), LuxThreshold(manager), SleepAbsenceDelay(manager)])
...
class SleepAbsenceDelay(ArmEntity, NumberEntity):
    _attr_name = "Sleep absence delay"
    _attr_native_min_value = 0
    ...
    def __init__(self, manager):
        super().__init__(manager, "sleep_absence_delay")

    @property
    def native_value(self):
        return self.manager.sleep_absence_delay_minutes

    async def async_set_native_value(self, value):
        options = {**self.manager.entry.options, CONF_SLEEP_ABSENCE_DELAY: int(value)}
        self.hass.config_entries.async_update_entry(self.manager.entry, options=options)
        await self.manager.async_evaluate()
