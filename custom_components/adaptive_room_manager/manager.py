...
class RoomManager:
    ...
    async def _save_sleep_state(self) -> None:
        await self._store.async_save(
            {"sleep_mode": self.sleep_mode, "snapshot": self._sleep_snapshot}
        )
    ...
    def _lights_changed_by_sleep_profile(self) -> set[str]:
        """Return only lights for which sleep mode will issue a command.

        Sleep lights configured ON are considered changed because brightness/color may
        be adjusted. Lights configured OFF, and normal lights excluded from the sleep
        profile, are included only when currently ON.
        """
    ...
    async def _apply_sleep_profile(self) -> None:
        ...
        to_off = {
            ...
            and not (
                entity_id in sleep_lights
                and profiles.get(entity_id, {}).get(CONF_SLEEP_LIGHT_ON, True)
            )
        }
        if to_off:
